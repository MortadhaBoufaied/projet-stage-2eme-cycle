...
if page=="Dashboard":
    ...
    if task=="Credit risk":
        ...
        if df is not None:
            ...
            if not auto_select:
                a,b,c=st.columns(3);model_type=a.selectbox("Model",sorted(AVAILABLE_CREDIT_MODELS),help="XGBoost and LightGBM are recommended for tabular data; alternatives remain for benchmarking.");review=b.slider("Review threshold",.10,.85,.50,.05);high=c.slider("High-risk threshold",.20,.95,.60,.05)
                    ...
                    try:
                        with st.status("Training credit model",expanded=True) as status:
                            if optimize_threshold:
                                st.write("Training with automatic threshold optimization");model,met=train_credit_with_optimal_threshold(mapped,model_type,hyperparams=None)
                            else:
                                model,met=train_credit(mapped,model_type,review_threshold=review,high_risk_threshold=high)
                            st.write("Saving version");registry.save(company,"credit",model,{"metrics":met,"mapping":mp,"data_summary":quality_report(mapped),"experiment":"baseline"});status.update(label="Model saved",state="complete")
                ...
                if review_h>high_h:validation.append("threshold");st.error("Review threshold cannot exceed the high-risk threshold.")
                if st.button("Train all models and select the best",type="primary",disabled=bool(errs or validation)):
                    ...
                    except Exception as e:st.error(f"Training all models failed. {e}")
                    if st.button("Grid search all models (find best params)",type="primary",disabled=bool(errs or validation)):
                        ...
                        except Exception as e:st.error(f"Grid search failed. {e}")
    else:
        ...
        if df is not None:
            ...
            if not auto_select:
                ...
                if st.button("Train all forecast models and select the best",type="primary",disabled=bool(errs or validation)):
                    ...
                    except Exception as e:st.error(f"Training all forecast models failed. {e}")
                    if st.button("Grid search all forecast models",type="primary",disabled=bool(errs or validation)):
                        ...
                        except Exception as e:st.error(f"Forecast grid search failed. {e}")
